...
router.post('/orders/:id/approve', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/orders/:id/reject', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/orders/:id/status', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/analytics/sales', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const [totalOrders, pendingOrders, approvedOrders, rejectedOrders, totalRevenue, recentOrders, topProducts] = await Promise.all([
            prisma.order.count(),
            prisma.order.count({ where: { status: 'PENDING' } }),
            prisma.order.count({ where: { status: { in: ['APPROVED', 'PROCESSING', 'SHIPPED', 'DELIVERED'] } } }),
